        ...
        .on("connect", (req, clientSocket, head) => {
          ...
          if (!this.hasLogged403) {
            const maskedPassword = this.targetProxy.password ? 
              ...
              '***'
            logger.debug(`CONNECT request for ${hostname}:${port} using proxy ${this.targetProxy.host}:${this.targetProxy.port}`)
          ...
          if (buffer.length >= 2 && buffer[0] === 0x05) {
            ...
            if (buffer.length >= 2 + methodCount) {
              ...
              if (buffer.length >= 5) {
                ...
                if (targetHost && targetPort && cmd === 0x01) { // CONNECT
                  ...
                  upstreamSocket.on('data', (data) => {
                    ...
                    if (requestSent && upstreamBuffer.length >= 10) {
                      ...
                      if (upstreamBuffer[1] === 0x00) {
                        ...
                        // Error
                        clientSocket.write(Buffer.from([0x05, upstreamBuffer[1], 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]))
                        clientSocket.end()
                        upstreamSocket.end()
                      }
                    }
                  ...
                  upstreamSocket.on('error', (err: any) => {
                    if (err.code === "ECONNREFUSED") {
                      if (!this.hasLoggedConnectionRefused) {
                        this.hasLoggedConnectionRefused = true
                        logger.error(`❌ SOCKS5 proxy connection refused: ${this.targetProxy.host}:${this.targetProxy.port}`)
                        logger.warn(`   ⚠️  Possible causes:`)
                        logger.warn(`   - Proxy server is down or offline`)
