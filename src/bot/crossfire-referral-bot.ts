...
export class CrossfireReferralBot {
  ...
  private async getBrowserVersion(path: string): Promise<string | null> {
    ...
    if (process.platform === "win32" && path.includes("chrome.exe")) {
      try {
        const result = execSync(`wmic datafile where name="${path.replace(/\\/g, "\\\\")}" get Version /value 2>nul`, execOpts)
  ...
  private async performDirectConnectionSecurityAudit(): Promise<void> {
    try {
      ...
      logger.info(`${methodEmoji} Connection Method: ${connectionMethod.method.toUpperCase()} (${connectionMethod.confidence}% confidence)`)
